...
export default function DetalleProyectoModal({
  ...
  const manejarGenerarPaquete = async () => {
    ...
    try {
      ...
      setErrorGeneracion("No se ha podido generar el documento. Inténtalo de nuevo.");
    } finally {
      setGenerando(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-slate-900/50 p-4 py-10">
      <div className="w-full max-w-3xl rounded-xl bg-white shadow-xl">
        <div className="flex items-start justify-between border-b border-slate-200 p-6">
          <div>
            <p className="text-sm text-slate-500">{proyecto.codigo} · {proyecto.categoria}</p>
            <h2 className="mt-1 text-2xl font-bold text-slate-900">{proyecto.nombre}</h2>
        ...
        <div className="max-h-[70vh] overflow-y-auto p-6">
          <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <Campo etiqueta="Estado" valor={<EstadoSaludDot estado={proyecto.estado} />} />
            ...
            <Campo etiqueta="Categoría estratégica" valor={<CategoriaBadge categoria={proyecto.categoria} />} />
            <Campo etiqueta="Objetivo estratégico" valor={proyecto.objetivoEstrategico} />
            <Campo etiqueta="Fecha prevista de fin" valor={formatearFecha(proyecto.fechaFinPrevista)} />
            <Campo
              etiqueta="Próximo gate"
              valor={
                proximoGate
                  ? `${proximoGate}${proyecto.fechaProximoGate ? ` — ${formatearFecha(proyecto.fechaProximoGate)}` : ""}`
                  : "Sin gates pendientes"
              }
            />
            {proyecto.categoria === "Protección de valor" && proyecto.exposicionRiesgo !== undefined && (
              <Campo etiqueta="Exposición al riesgo" valor={formatearEuros(proyecto.exposicionRiesgo)} />
            )}
            {proyecto.categoria === "Creación de valor" && proyecto.beneficioEsperado !== undefined && (
              <Campo etiqueta="Beneficio esperado" valor={formatearEuros(proyecto.beneficioEsperado)} />
            )}
          </section>
          ...
          {proyecto.estado === "Cancelado" && proyecto.motivoCancelacion && (
            <section className="mt-6 rounded-lg border border-rose-200 bg-rose-50 p-4">
              <h3 className="text-sm font-semibold text-rose-700">Motivo de cancelación</h3>
              <p className="mt-2 text-sm text-rose-700">{proyecto.motivoCancelacion}</p>
            </section>
          )}
          ...
          <section className="mt-6 rounded-lg bg-slate-50 p-4">
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-5">
              ...
              <Campo etiqueta="Actuals" valor={formatearEuros(proyecto.importeGastado)} />
              <Campo etiqueta="ETC" valor={formatearEuros(proyecto.etc)} />
              <Campo etiqueta="EAC" valor={formatearEuros(eac)} />
              <Campo etiqueta="Variación" valor={<VariacionBadge porcentaje={variacion} />} />
            </div>
          </section>

          <section className="mt-6">
            <h3 className="text-sm font-semibold text-slate-900">Progresión de gates</h3>
            <div className="mt-3"><GateProgresionPills proyecto={proyecto} /></div>
          </section>

          <section className="mt-6">
            <h3 className="text-sm font-semibold text-slate-900">Desglose de costes por fase</h3>
            <div className="mt-3"><FaseCostesTabla proyecto={proyecto} /></div>
          </section>

          <section className="mt-6">
            <h3 className="text-sm font-semibold text-slate-900">Descripción</h3>
            <p className="mt-2 text-sm text-slate-700">{proyecto.descripcion}</p>
          </section>
          ...
          <section className="mt-6 flex flex-wrap items-center gap-3 border-t border-slate-200 pt-6">
            ...
            {errorGeneracion && <p className="text-sm text-rose-600">{errorGeneracion}</p>}
          </section>
        </div>
      </div>
    </div>
