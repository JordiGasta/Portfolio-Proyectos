...
export default function DemandaSemestralChart({
  ...
  const fuera = presupuestoFueraDeVentana(proyectos, semestres);
  ...
  return (
    <article className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <p className="mt-1 text-xs text-slate-400">
        El presupuesto y el gasto de cada fase se sitúan en el semestre de
        su fecha de fin (corte el 30 de junio). Haz clic en un semestre
        para ver el desglose por proyecto.
      </p>

      <div className="mt-8 flex items-end justify-around gap-6">
        {datos.map((dato) => {
          const alturaPresupuesto = Math.round(
            (dato.presupuesto / maximo) * ALTURA_GRAFICO,
          );
          const alturaGastado = Math.round(
            (dato.gastado / maximo) * ALTURA_GRAFICO,
          );
          ...
          return (
            <button
              ...
              className={`flex flex-1 flex-col items-center rounded-md p-2 transition ${
        ...
        <div className="mt-6 rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
          <p>
            <span className="font-medium text-slate-700">
              {formatearEuros(Math.round(fuera))}
            </span>{" "}
            de presupuesto corresponde a fases cuya fecha de fin queda
            fuera de {etiquetaSemestre(semestres[0])} –{" "}
            {etiquetaSemestre(semestres[semestres.length - 1])}, por lo que
    ...
    >
      <div className="w-full max-w-2xl rounded-xl bg-white shadow-xl">
        ...
        <div className="max-h-[65vh] overflow-y-auto p-6">
            ...
            <tbody className="divide-y divide-slate-100">
              {datos.contribuciones.map((c) => (
                <tr key={c.proyecto.id}>
                  ...
                  <td className="py-2 text-right text-slate-900">
                    {formatearEuros(Math.round(c.gastado))}
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          {datos.contribuciones.length === 0 && (
            <p className="text-sm text-slate-500">
              No hay proyectos en este semestre.
            </p>
          )}
        </div>
      </div>
